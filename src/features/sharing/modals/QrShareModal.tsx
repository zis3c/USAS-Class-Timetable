...
import { copyTextToClipboard, sanitizeSingleLine, sanitizeTextForShare } from '@/shared/lib/security';
...
export default function QrShareModal({
  ...
  const safeMatricNo = sanitizeSingleLine(matricNo, 32);
      ...
      <div className={`rounded-xl w-full max-w-[92vw] sm:max-w-sm border pt-3 px-4 sm:px-5 pb-5 space-y-4 relative transition-all duration-200 transform ${
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-3 pt-0 min-w-0">
          ...
          <div className="min-w-0 text-left">
            ...
            <p className={`text-xs font-semibold truncate ${isLight ? 'text-amber-700' : 'text-amber-400'}`}>{safeStudentName} ({safeMatricNo})</p>
          </div>
        </div>
          ...
          <div className={`p-3.5 rounded-xl border inline-block shadow-inner ${
            ...
            <div className="relative w-40 h-40 sm:w-44 sm:h-44 mx-auto">
              ...
              {qrUrl ? (
                ...
                <div className={`w-40 h-40 sm:w-44 sm:h-44 rounded-lg flex items-center justify-center text-[10px] font-semibold ${
                  isLight ? 'bg-slate-100 text-slate-400' : 'bg-white/[0.04] text-white/40'
                }`}>
                  QR unavailable
                </div>
              )}
